#!/bin/bash
# Builder modified: Mon July 06, 2020 @ 03:54:30 EDT
...
confirm="no"
if [ "$(pwd)" != "$(git rev-parse --show-toplevel 2>&1)" ]; then
until [ "$confirm" = "yes" ]; do
    confirm="no";
    echo ""
    read -p 'Specify SiteName (eg. "Google"): ' bld_sitename;
    read -p -N 3 'Specify Three-Letter Abbreviation (eg. "goo"): ' bld_abbrev;
